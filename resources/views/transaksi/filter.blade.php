                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <select name="bank" class="form-select" aria-label=" Default select example"
                                                id="selectBank" value="{{ Request::get('bank') }}">
                                            ...
                                            <select name="mitra" class="form-select" aria-label="Default select example"
                                                ...
                                                @if (!empty($list_mitra))
                                                    @foreach ($list_mitra as $mitra)
                                                        <option value="{{ $mitra->kode_cid }}" class="mitraOption"
                                                            {{ $mitra->kode_cid == Request::get('mitra') ? 'selected' : '' }}>
                                                            {{ $mitra->nama_cid }}
                                                        </option>
                                                    @endforeach
                                                @endif
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <select name="produk" class="form-select" id="selectProduk"
                                                aria-label="Default select example">
                                                <option selected value=''>-- Pilih Produk --</option>
                                                @if (!empty($list_produk))
                                                    @foreach ($list_produk as $produk)
                                                        <option value="{{ $produk->produk->kode_produk }}"
                                                            class="produkOption"
                                                            {{ $produk->produk->kode_produk == Request::get('produk') ? 'selected' : '' }}>
...
@section('script')
    ...
    <script>
        // $('#selectMitra').prop('disabled', 'disabled');
        ...
        $(document).on('change', '#selectBank', function() {
            ...
            $.get('/bank/' + $(this).val() + '/getMitra', function(data) {
                $.each(data.mitra, function(index, value) {
                    $('#selectMitra').append(
                        '<option class="mitraOption" value="' + data.mitra[index].kode_cid +
                        '">' + data.mitra[index].nama_cid + '</option>'
                    );
                });
            })
        ...
        $(document).on('change', '#selectMitra', function() {
            ...
            $.get('/mitra/' + $(this).val() + '/getProduk', function(data) {
                ...
                $.each(data.produk, function(index, value) {
                    console.log(data.produk);
                    $('#selectProduk').append(
                        '<option class="produkOption" value="' + data.produk[index].produk
                        .kode_produk + '">' + data.produk[index].produk.nama_produk +
                        '</option>'
                    );
                });
            });
